...
import { exercises, patients } from '../../data.js'
import { LineChart, Line, XAxis, YAxis, Tooltip, ResponsiveContainer, CartesianGrid } from 'recharts'

const patient = patients[0] // Arjun Mehta as logged-in patient
const days = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun']

export default function PatientDashboard() {
  const navigate = useNavigate()
  const patientExercises = exercises.filter(ex => patient.exercises.includes(ex.id))
  const chartData = patient.progress.map((score, i) => ({ day: days[i], score }))
  ...
  return (
    <div style={{ background: '#f5f5f7', minHeight: '100vh', paddingBottom: 80, fontFamily: '"Inter", sans-serif' }}>
      ...
      <div style={{ background: '#000', padding: '48px 24px 28px' }}>
        ...
        <p style={{ fontSize: 14, color: '#6e6e73', marginTop: 6 }}>{patient.condition}</p>

        {/* Score + streak row */}
        <div style={{ display: 'flex', gap: 16, marginTop: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <ScoreBadge score={patient.score} size={48} />
            <div>
              <div style={{ fontSize: 12, color: '#6e6e73' }}>Overall score</div>
              <div style={{ fontSize: 14, fontWeight: 600, color: '#fff' }}>
                {patient.score >= 75 ? 'Great progress' : patient.score >= 50 ? 'Keep going' : 'Needs work'}
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div style={{ fontSize: 14, fontWeight: 600, color: '#34c759' }}>{patient.streak} days 🔥</div>
          </div>
        </div>
      </div>
